...
import { Search, Filter, Users, Upload, Trash2, Eye, Archive, UserX, RotateCcw, FileText } from "lucide-react"
import { generateEnrollmentPdf } from "@/lib/utils/enrollment-pdf"
  ...
  const handleGenerateEnrollment = async (studentId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `A declaração de matrícula de ${student.name} foi baixada`,
      })
    } catch (error) {
      toast({
              ...
              >
                <CardContent className="p-2 sm:p-3">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleGenerateEnrollment(student.id)}
                    disabled={generatingPdfId === student.id}
                    className="w-full gap-0.5 sm:gap-1 text-[10px] sm:text-xs h-6 sm:h-7 px-1 sm:px-2 mb-1 sm:mb-1.5 text-primary hover:text-primary hover:bg-primary/10 border-primary/30"
                    title="Baixar declaração de matrícula em PDF"
                  >
                    <FileText className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                    {generatingPdfId === student.id ? "Gerando..." : "Matrícula"}
                  </Button>
