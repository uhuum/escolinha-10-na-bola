...
export default function TrainerBirthdaysPage() {
  ...
  const coachStudents = useMemo(() => {
    return students.filter((student) => {
      ...
      // Check if student has scheduleConfigs (new format)
      if (student.scheduleConfigs && student.scheduleConfigs.length > 0) {
        return student.scheduleConfigs.some((config) =>
          coachClasses.some(
            (classInfo) => config.schedule === classInfo.schedule && classInfo.days.includes(config.day),
          ),
        )
      }

      // Fallback to old format
      return coachClasses.some(
        (classInfo) =>
          student.classSchedule === classInfo.schedule &&
          student.classDays?.some((day) => classInfo.days.includes(day)),
      )
    })
  }, [students, coachClasses])
      ...
      <div className="space-y-6">
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12 sm:py-16">
            <Cake className="h-16 w-16 sm:h-20 sm:w-20 text-muted-foreground mb-4 opacity-50 animate-pulse" />
    ...
    <div className="space-y-6 sm:space-y-8">
      <div>
        ...
        <p className="text-sm sm:text-base lg:text-lg text-muted-foreground px-11">
          Aniversários dos alunos das suas turmas
        </p>
      </div>
      ...
      {birthdaysThisMonth.length > 0 ? (
        <Card className="border-2 hover:border-primary/50 transition-colors bg-gradient-to-br from-primary/5 to-accent/5">
          <CardHeader>
            ...
            <CardDescription className="text-base">Alunos das suas turmas com aniversário neste mês</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-2">
          <CardContent className="flex flex-col items-center justify-center py-12 sm:py-16">
            ...
            <p className="text-sm sm:text-base text-muted-foreground text-center">
              Não há alunos das suas turmas fazendo aniversário em {displayMonth}
            </p>
          </CardContent>
        </Card>
      )}
